...
- `output.min_matched_prefix_length`, `output.min_matched_suffix_length`, and `output.save_match_mode` let you keep a broader grouped search active while only saving stronger matches such as `5x5+`
- when the save filter is enabled, `gpu/run.ps1` prints `Save filter   : prefix >= X and suffix >= Y` before the iteration updates begin
